...
export function ImageSlider({ images, alt }: { images: string[]; alt: string }) {
  ...
  if (images.length === 0) {
    return <div className="flex aspect-video items-center justify-center rounded-3xl bg-brand-light text-muted">사진 준비중</div>;
  }
  const prev = () => setI(v => (v - 1 + images.length) % images.length);
  const next = () => setI(v => (v + 1) % images.length);
  return (
    <div>
      <div className="relative aspect-video overflow-hidden rounded-3xl border border-hairline bg-brand-light">
        ...
        <div className="mt-2 flex gap-2 overflow-x-auto">
          {images.map((src, idx) => (
            <button key={src} type="button" onClick={() => setI(idx)} aria-label={`사진 ${idx + 1} 보기`} aria-current={i === idx}
                    className={`relative h-16 w-16 overflow-hidden rounded-xl border-2 transition ${i === idx ? 'border-brand' : 'border-transparent opacity-70 hover:opacity-100'}`}>
