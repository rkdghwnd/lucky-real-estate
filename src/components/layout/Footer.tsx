...
export function Footer() {
  ...
  return (
    <footer className="mt-20 border-t border-hairline bg-[#f5f6f8] text-muted">
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_auto]">
          <div>
            ...
            <p className="text-sm leading-7">
              전화:{' '}
              <a href={siteConfig.phoneHref} className="font-medium text-brand transition hover:underline">{siteConfig.phone}</a>
          ...
          <div className="md:text-right">
            ...
            <nav className="mt-4 flex flex-col gap-2.5 text-sm md:items-end">
              ...
              <Link href="/listings" className="transition hover:text-brand">매물검색</Link>
              <Link href="/about" className="transition hover:text-brand">회사소개</Link>
              <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="transition hover:text-brand">
                네이버 지도에서 사무소 위치 보기
              </a>
            </nav>
          </div>
