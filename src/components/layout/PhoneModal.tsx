    ...
    <>
      ...
      {open && (
        <div role="dialog" aria-modal="true" aria-label="전화상담"
          ...
          <div className="w-full max-w-sm rounded-3xl bg-canvas p-6 text-center" onClick={e => e.stopPropagation()}>
            <p className="text-lg font-bold text-ink">{siteConfig.name}</p>
            <p className="mt-1 text-muted">{siteConfig.businessHours}</p>
            <a href={siteConfig.phoneHref} className="mt-4 block rounded-full bg-brand py-4 text-2xl font-bold text-white transition hover:bg-brand-dark">📞 {siteConfig.phone}</a>
