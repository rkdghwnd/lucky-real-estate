...
import { buildListingJsonLd, buildBreadcrumbJsonLd } from '@/lib/seo';
...
export function ListingDetail() {
  ...
  const { data: l, isLoading, isError } = useListing(slug);

  useEffect(() => {
    if (l) document.title = `${l.title} | 행운부동산`;
  }, [l]);

  if (isLoading) {
    return <div className="mx-auto max-w-6xl px-4 py-24 text-center text-muted">불러오는 중…</div>;
  }
  if (isError) {
    return <div className="mx-auto max-w-6xl px-4 py-24 text-center font-semibold text-muted">매물을 불러오지 못했습니다.</div>;
  }
